...
async def set_managers(
    ...
    # Диалоги, закреплённые за тем, кого в новом списке менеджеров аккаунта нет,
    # иначе повисают невидимыми никому: сам он аккаунт может вообще не видеть
    # (или видеть, но диалог не «свой»), а оставшимся менеджерам диалог не
    # «свой» и не «ничейный» — не подходит ни под одно условие видимости (D-26).
    # Это не только явно снятые из account_managers: диалог мог достаться и
    # тому, кто в этом списке никогда не состоял — например, руководителю,
    # ответившему лично («кто первый ответил, тот и ведёт») ещё до того, как
    # на аккаунт вообще назначили менеджера. Смотрим не на «кого убрали»,
    # а на «кто сейчас отвечает за диалог и не входит в новый список» — иначе
    # именно такие диалоги остаются висеть на прежнем «хозяине» навсегда.
    prev_owners = sorted(
        {
            row[0]
            for row in (
                await db.execute(
                    select(Conversation.responsible_id).where(
                        Conversation.account_id == account.id,
                        Conversation.responsible_id.isnot(None),
                        Conversation.responsible_id.not_in(new_ids),
                    )
                )
            ).all()
        }
    ...
    if prev_owners:
        result = await db.execute(
            ...
            .where(
                Conversation.account_id == account.id,
                Conversation.responsible_id.in_(prev_owners),
            )
        ...
        if released:
            await log_event(
                ...
                entity_id=account.id,
                actor=admin,
                before={"user_ids": prev_owners},
                after={"conversations": released},
            )

    # На аккаунте остался ровно один менеджер — значит номер его, и все ничейные
    # диалоги (включая только что освобождённые блоком выше) закрепляются за
    # ним. Иначе после подтяжки истории или передачи чужого аккаунта человек
    # видел бы сотни «свободных» чатов в общей очереди вместо своей переписки.
    # При двух и более менеджерах на аккаунте освобождённые диалоги специально
    # остаются «ничейными» — авторазбирать, кому из нескольких они достанутся,
    # было бы гаданием, поэтому они уходят в общую очередь.
    adopted = 0
    if len(new_ids) == 1:
        result = await db.execute(
            update(Conversation)
